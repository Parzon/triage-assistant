...
--   make bench-rag-filter   (docs/handbook/rag.md, "A team filter")
INSERT INTO teams (slug, name)
SELECT 'bench-t' || g, 'Bench team ' || g FROM generate_series(1, 100) g
ON CONFLICT (slug) DO NOTHING;

INSERT INTO runbooks (team_id, title, body, body_sha256, embedding_key)
SELECT id, 'bench', 'bench', 'bench', 'bench-random' FROM teams WHERE slug LIKE 'bench-t%'
...
SELECT r.id, r.team_id, n, 'bench ' || n, 'bench content',
...
WHERE r.title = 'bench' AND r.embedding_key = 'bench-random';
