...
import { ApiError, type AssistantStatus, type Citation, streamChat } from '../lib/api'
...
/** `assistant`: the off switch's state (App fetches it); unknown counts as on. */
export function Chat({ assistant }: { assistant?: AssistantStatus }) {
  ...
  async function ask() {
    ...
    try {
      ...
      if (err instanceof DOMException && err.name === 'AbortError') {
        ...
        // Switched off since the page loaded: fetch why, for the notice.
        if (err.code === 'assistant_disabled') void queryClient.invalidateQueries({ queryKey: ['assistant'] })
      } else {
  ...
  return (
    <section className="panel" aria-labelledby="chat-title">
      ...
      {off && (
        <p className="notice">
          The assistant is switched off{assistant?.reason ? `: ${assistant.reason}` : ''}. Alerts and runbooks
          still work.
        </p>
      )}
      <form
      ...
      >
        ...
        {busy ? (
          ...
          <button key="ask" type="submit" disabled={!question.trim() || off}>
            Ask
          </button>
        )}
      </form>
