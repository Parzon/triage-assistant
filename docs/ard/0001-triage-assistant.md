...
**Date:** 2026-09-24
**Related:** [PRD-0001](../prd/0001-triage-assistant.md), [ADR-0001 to ADR-0016](../adr/), [RFC-0001](../rfc/0001-answers-grounded-in-runbooks.md), [RFQ-0001](../rfq/0001-llm-inference.md)
...
- **The assistant:** an AI model answers questions about the alerts the
  asker may see, streaming.
- **Sign-in:** the company's OIDC identity provider; roles come from its
  groups claim.

```
 on-call engineers ──HTTPS──► triage-assistant ──► model provider (OpenAI-compatible API)
 Alertmanager ─────webhook──►        │         ──► identity provider (OIDC)
                                     └──► its own Postgres (alerts, teams, users, sessions)
```
...
| Latency | alert list p95 < 300 ms; time to first word p95 < 3 s | `http_request_duration_seconds`, `llm_time_to_first_token_seconds` | ✅ ~4 ms at 500 req/s; 0.21 s with a local model |
...
| AI quality and safety | safety cases 100%; quality ≥ 80%; a leak rate < 1.5% (95% bound) | `make evals` with a calibrated judge (ADR-0016) | ✅ prompt v5: 10/10 per case; 0 leaks in 200 |
...
| Security | no known attack left without a tested control | the security chapter's attack table | ✅ 15 attacks, each with a control; 14 with a test (a leaked backup is covered by hashing, untested) |
...
| db | PostgreSQL 17 | the data, with row-level security | 0014 |
...
- **the assistant:** visible alerts → prompt → streamed answer;
...
|---|---|---|---|---|
| Alert text | internal; may contain hostnames, and credentials that systems print | Postgres | **yes: to the model provider**, the asker's visible alerts only | forever today (F8 📘) |
| Questions and answers | internal | not stored; not logged | questions go to the model provider | only for the answer's duration |
...
- **Threat model:** 15 attacks, each mapped to a control, and 14 to a
...
| What reaches it | the fixed instructions, then up to 20 of the asker's visible alerts (300 characters each), then the question |
...
| How quality is measured | 14 versioned eval cases (grounding, refusal, injection, isolation). A judge from another model family, calibrated on 21 labelled answers. A gate before every prompt or model change (ADR-0016) |
| Prompt injection | alert text is untrusted. Injection cases are safety-gated (100%). v5 leaked its instructions 0 times in 200, where v4 leaked 5 in 200. The prompt holds no secrets; the architecture bounds the impact |
...
| Alert text containing a credential reaches the provider | medium | high | zero-retention terms; 📘 redacting secret patterns before the prompt | service owners + security |
...
- **Decisions:** ADR-0001 to ADR-0016.
...
- Secret redaction before the prompt.
